# tasks/cansi-patterns.sh
function btask.cansi-patterns.run() {
    # if [ "$1" == '--help' ]; then
    ...
    templates[1]="${HOME}/developer/template-code/cansi-pattern/cansi-pattern-first-class-adt"
    templates[2]="${HOME}/developer/template-code/cansi-pattern/cansi-pattern-state"
    templates[3]="${HOME}/developer/template-code/cansi-pattern/cansi-pattern-state-loop"
    templates[4]="${HOME}/developer/template-code/cansi-pattern/cansi-pattern-strategy"
        ...
        if b.opt.check_required_args; then
            ...
            if b.opt.has_flag? --template; then
                for f in ${DIR}/*.yml; do
                    ...
                    if [ -e "$orig" ]; then
                        if [ -e "$dest" ]; then
                            echo `tput bold tput setb 3` action: `tput setaf 3` "File ${dest##*/} already exists!"  `tput sgr0`
                        else
            ...
            echo USAGE:
            echo 'xp new cansi-patterns {-n|--number} <number> [-p|--print|-s|--save] [-y|--yaml] <file>'
            echo 'xp new cansi-patterns -l'
            echo 'xp new cansi-patterns {-n|--number} <number> {-t|--template}'
        fi
    fi
